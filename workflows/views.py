...
from .models import ActionItem, Step, StepCompletion, Wizard
...
def workflows_dashboard(request):
    context = {
        "assigned_wizards": _populate_wizard_list(
            request.user, Wizard.objects.get_user_assigned_wizards(request.user)
        )
    }

    return render_to_string(
        "workflows/dashboard.html", context=context, request=request
    )
...
def index(request: WSGIRequest) -> HttpResponse:
    ...
    wizards = _populate_wizard_list(
        request.user, Wizard.objects.get_user_wizards(request.user)
    )

    context = {"assigned_wizards": assigned_wizards, "wizards": wizards}
...
@login_required
@permission_required("workflows.basic_access")
def view_wizard_by_permalink(
    request: WSGIRequest, permalink: str, step_id: int = 0
) -> HttpResponse:
...
@login_required
@permission_required("workflows.basic_access")
def view_wizard_by_id(
    request: WSGIRequest, wiz_id: int, step_id: int = 0
) -> HttpResponse:
...
def _view_wizard(request: WSGIRequest, wizard: Wizard, step_id: int) -> HttpResponse:
    ...
    if request.user not in wizard.users:
    ...
    for i, step in enumerate(wizard.steps.all()):
        step_dict = {
            "id": i + 1,
            "step": step,
            "complete": step.is_complete(request.user, wizard),
            "checks": _calculate_step_checks(request.user, step),
            "visible": (
                step.visibility.is_complete(request.user)
                if step.visibility is not None
                else True
            ),
        }
    ...
    if c and not steps[update_step]["complete"]:
        StepCompletion.objects.update_or_create(
            user=request.user,
            wizard=wizard,
            step=Step.objects.get(pk=steps[update_step]["step"].pk),
        )
    ...
    if current_step is not None:
        step_pct_complete = steps[current_step]["step"].pct_complete(
            request.user, wizard
        )
    else:
        step_pct_complete = 1
        ActionItem.objects.update_or_create(
            user=request.user, wizard=wizard, defaults={"completed": True}
        )
    ...
    if current_step is None:
        ...
        context["body_text"] = _render_body_or_default(
            steps[current_step]["step"].body, context
        )
